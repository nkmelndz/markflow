...
export const usePreview = (content: string, viewMode: 'marp' | 'markdown', onLineClick?: (line: number) => void) => {
  ...
  const handlePreviewClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (lineElement) {
      // Ignore clicks on the slide container itself (SECTION) for Marp
      if (viewMode === 'marp' && lineElement.tagName === 'SECTION') return;
